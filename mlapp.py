...
diction={1:"Normal",2:"Ischemic changes (Coronary Artery Disease)",3:"Old Anterior Myocardial Infarction",4:"Old Inferior Myocardial Infarction ",
...
x_train,test,tar,yt=train_test_split(ooo,target1,test_size=0.1,random_state=101)   
# Rescaling the Data
...
def outliers(dataframe):
 ...
 # print(*l)
  ...
  plot(l[:10],dataframe)
  for i in l:
    d[i]=detect_outlier(dataframe[i],dataframe,i)   # we are detecting the outliers in each and every columns
...
#st.write(obtained.isnull().sum())
#obtained.dropna(inplace=True)
#st.header("Feature Extraction")    
#pca = PCA(n_components = 50) 
#pca.fit(obtained) 
#x_pca = pca.transform(obtained) 

#st.write(x_pca.shape)
#df_comp = pd.DataFrame(pca.components_, columns =obtained.columns) 
...
if st.button("Model"):
   ...
   # o.drop("diagnosis",inplace=True)
    ...
    base_model = RandomForestClassifier(n_estimators = 600, random_state = 42,min_samples_leaf=1,max_features="sqrt",max_depth=40,bootstrap=False)
    base_model.fit(xtrain,ytrain)
    base_accuracy = evaluate(base_model, xtest,ytest)

    st.write("accuracy is ",base_accuracy)

    st.write("Prediction for a single item",base_model.predict(np.array(dataset.loc[1]).reshape(1,-1)))  
    st.write("He is ",diction[base_model.predict(np.array(dataset.loc[1]).reshape(1,-1))[0]])
    st.write(np.array(dataset.loc[1]).reshape(1,-1)[0])
#dataset=o.select_dtypes(exclude=["object"])
#xtrain,xtest,ytrain,ytest=train_test_split(obtained,target2,test_size=0.1,random_state=101)
#base_model1 = RandomForestClassifier(n_estimators = 600, random_state = 42,min_samples_leaf=1,max_features="sqrt",max_depth=40,bootstrap=False)
#base_model1.fit(xtrain,ytrain)
#base_accuracy = evaluate(base_model1, xtest,ytest)
...
n111=np.array(l)
st.write(age1,gender,height,weight,qrs_duration,heart,qtinterval,tinterval,pinterval,qrs)
...
#k=np.hstack((n111,np.array(dataset.loc[1]).reshape(1,-1)[0][5:]))
#st.write(k)
#df=pd.DataFrame()
#tp=np.hstack((n111,np.array(dataset.loc[1]).reshape(1,-1)[0][5:]))
n111=np.array(l)
data_to_append = {}
for i in range(len(dataset.columns)):
    data_to_append[dataset.columns[i]] = n111[i]
#st.write(data_to_append.items(),dataset.columns)

df = pd.DataFrame(data_to_append,index=[0])
#st.write(df)
hh=base_model.predict(np.array(df.loc[0]).reshape(1,-1))
st.write(hh,type(hh))
st.write("He has ",diction[hh[0]])
